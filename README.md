...
For analyzing additional malware that targets different Android SDK versions, place the corresponding SDK's android.jar file inside the android_jar directory.
For example, if you are to analyze malware targeting Android 14 (SDK version 34), you should:
 ...
 4. Place the android.jar file in `DVa/android_jar/platform-34`.


### Usage

Next, we show an example of executing DVa to extract the targeted victims, abuse vectors, and persistence mechanisms of the cerberus a11y malware.

1. `cd` to DVa's directory.
2. Run DVa's malware analysis module against the malware:
```sh
java -jar static_analysis.jar samples/cerberus.apk $OUTPUT_PATH
```
3. Retrieve the malware analysis report at `$OUTPUT_PATH/cerberus.json`.
...
This is organized as the package names of the victim apps or system services that the malware targets.
In specific, it is the package name contained in each accessibility event that the malware listens for and parses.
Each package name is additionally accompanied by the trace of constraints that it is resolved from.
2. Abuse vectors of the malware.
This is organized as one of the seven a11y abuse vectors the malware uses to abuse each victim.
...
3. Persistence mechanisms of the malware.
This contains one of the six a11y persistence mechanisms the malware adopts to hinder the user's removal of the malware.
Each mechanism is accompanied by traces of the mechanism triggers and a11y action responses to its triggers.
