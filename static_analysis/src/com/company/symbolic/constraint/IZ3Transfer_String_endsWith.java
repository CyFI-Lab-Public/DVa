...
public class IZ3Transfer_String_endsWith implements IZ3Transfer {

	static String tagName = "String.endsWith";
	...
	public void handle(IRunTimeManager rtm, ProcessExpr pExpr, InvokeExpr invokeExpr, Local target, Local base, String sign, List<Value> args) {
		...
		if (rtm.getRttd().contains(base)) {
			...
			if (args.get(0) instanceof StringConstant) {
				...
				LogFileUtil.helpInfo(String.format("String.endsWith:(%s) arg not String", rtm.getRttd().getCurrentMethod()));
			}

			rtm.getRttd().addTag(target, new ValueTag(tagName, arg));
		}
	}
	...
	public String transfer(Constraint cons) throws JSONException {
		...
		savetoJson(cons);
		return String.format("%s String.endsWith %s -- (%s)%s", source, tag.getData(), cons.getCondition(), cons.isSatisfy());
	}
	...
	public void savetoJson(Constraint cons) throws JSONException {
		...
		js.append("consName", "String_endsWith");
