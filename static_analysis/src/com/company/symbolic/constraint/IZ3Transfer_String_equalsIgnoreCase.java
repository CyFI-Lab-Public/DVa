...
public class IZ3Transfer_String_equalsIgnoreCase implements IZ3Transfer {

	static String tagName = "String.equalsIgnoreCase";
	...
	public void handle(IRunTimeManager rtm, ProcessExpr pExpr, InvokeExpr invokeExpr, Local target, Local base, String sign, List<Value> args) {
		...
		if (args.get(0) instanceof StringConstant && rtm.getRttd().contains(base)) {
			...
			MyLog.info("String_equalsIgnoreCase", "Not StringConstant arg " + args.get(0));
		}
	}
	...
	public String transfer(Constraint cons) throws JSONException {
		...
		savetoJson(cons);
		return String.format("'%s' equalsIgnoreCase '%s' -- (%s)%s", source, tag.getData(), cons.getCondition(), cons.isSatisfy());
	}
	...
	public void savetoJson(Constraint cons) throws JSONException {
		...
		js.append("consName", "String_equalsIgnoreCase");
