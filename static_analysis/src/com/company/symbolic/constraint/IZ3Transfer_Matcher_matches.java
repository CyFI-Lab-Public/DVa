...
public class IZ3Transfer_Matcher_matches implements IZ3Transfer {
	...
	public String transfer(Constraint cons) throws JSONException {
		...
		ITag tag = cons.getTag("Patterns.Pattern");
		if (tag != null) {
			savetoJson(cons);
			return String.format("%s Pattern %s -- (%s)%s", source, tag.getData(), cons.getCondition(), cons.isSatisfy());
		} else {
	...
	public void savetoJson(Constraint cons) throws JSONException {
		...
		js.append("consName", "Matcher_matches");
		js.append("consArg", cons.getTag("Patterns.Pattern").getData());
